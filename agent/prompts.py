...
def writer_methods(
    ...
    specs = [
        (
            ...
            "pack's anchor list) OR a [PLACEHOLDER:...] slot when the "
            "operational detail is not yet pinned. Use planned-analysis voice "
            "('will be', 'is designed to', 'pre-specifies') throughout — no "
            ...
            "PICO/PECO terms appropriate to the domain. The topic pack lists "
            "the primary intervention names; analogs/derivatives are NOT "
            "eligible for the primary pooled analysis and are retained only "
            "for a pre-specified translational sensitivity layer where the "
            "topic pack supplies translational-only interventions. Scope "
            "vocabulary must match the topic pack's preferred_terms.\n"
            ...
            "numeric thresholds in code; use original-unit extraction with "
            "post-hoc category sensitivity.\n"
            "5. RISK-OF-BIAS / QUALITY — select the framework appropriate to "
            "the study designs in scope and cite the topic-pack anchor that "
            "matches. For ANIMAL intervention studies prefer SYRCLE's RoB "
            "tool ([CIT:hooijmans-2014-syrcle|method-citation]) plus the "
            "ARRIVE 2.0 reporting checklist "
            "([CIT:percie-du-sert-2020-arrive|method-citation]). For human "
            "RCTs use RoB2 or ROBINS-I (if anchors are present in the pack). "
            "GRADE ([CIT:schunemann-grade|method-citation]) is optional for "
            "evidence-certainty framing, not the primary RoB tool for animal "
            "studies. State how disagreements are arbitrated.\n"
            ...
            "If the topic pack lists translational-only interventions, "
            "explicitly include a translational sensitivity layer that "
            "examines analog evidence at appropriate endpoints WITHOUT "
            "pooling it with the primary lifespan corpus.\n"
