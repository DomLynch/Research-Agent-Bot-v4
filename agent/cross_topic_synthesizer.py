...
The gate is deliberately structural: a lead synthesis can publish only
when multiple topic memos share the same alpha cue and each supporting
topic has bound receipts. The model may sharpen prose, but it cannot
create density.
"""
...
_WEAK_LABELS = frozenset({
    "curation_needed", "evidence_binding_failed", "discard", "no_signal",
})
...
def _fallback_sections(gate: SynthesisGate) -> dict[str, Any]:
    ...
    return {
        "cross_topic_thesis": thesis,
        "why_surprising": (
            "The shared pattern appears across separate topic runs rather "
            "than inside one isolated evidence pool, which makes it a "
            "higher-level lead candidate instead of another Top 5 item."
        ),
        "strongest_counter_thesis": (
        ...
        "what_this_changes": (
            "Treat the next review cycle as a cross-topic extraction "
            "campaign: test the shared pattern directly instead of "
            "publishing each topic in isolation."
        ),
        "what_would_falsify_it": (
            "The synthesis weakens if one or more supporting topics lose "
            "their bound receipts or if new source audit shows the shared "
            "cue was only a wording artifact."
        ),
        "next_extraction_campaign": [
            item
...
def _provenance(
    ...
    bundle = "\n".join(
        f"{m.run_dir.name}:{_sha256((m.run_dir / 'alpha_memo.md').read_text(encoding='utf-8'))}"
        for m in memos
        if (m.run_dir / "alpha_memo.md").exists()
    )
    citation = (
    ...
    return [
        ...
        f"- **Input bundle SHA-256:** `{_sha256(bundle)}`",
...
def render_cross_topic_memo(
    ...
    *,
    ...
    body = "\n".join(lines) + "\n"
    return "\n".join([body, *_provenance(output_path, list(gate.supporting), body)]) + "\n"
