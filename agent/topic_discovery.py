...
def _fetch_fullraw_topic_papers(topic: str, *, client: httpx.Client, limit: int = 25) -> list[dict[str, Any]]:
    ...
    payload = {"query": topic.replace("_", " ")[:1024], "limit": limit, "rank_mode": "relevance", "cache_only": True, "queue_if_missing": True, **({"priority": True} if os.environ.get("TOPIC_DISCOVERY_FULLRAW_PRIORITY", "").strip().lower() in {"1", "true", "yes", "on"} else {})}
    ...
    for _ in polls:
        ...
        if not attempts_raw:
            ...
            if remaining <= timeout:
                break
            time.sleep(min(wait_s or 0.1, remaining - timeout))
        elif wait_s:
            time.sleep(wait_s)
    ...
    for item in items:
        ...
        out.append({"doi": doi, "pmid": item.get("pmid"), "pmcid": item.get("pmcid"), "paper_id": item.get("paper_id") or item.get("id"), "title": title, "journal": item.get("journal") or item.get("venue") or item.get("source_name"), "publication_year": item.get("publication_year") or item.get("year"), "fwci": item.get("fwci") or 1.0, "cited_by_count": item.get("cited_by_count") or item.get("citation_count") or 0, "quality_score": item.get("quality_score") or 70.0, "url": f"https://doi.org/{doi}" if doi else item.get("url"), "fullraw_shard_receipt": dict(receipt), "source_fact": ({k: item.get(k) for k in ("id", "canonical_phrase", "population", "intervention", "comparator", "endpoint", "metric", "source_excerpt") if item.get(k) not in (None, "")} | {"source_tier": item.get("source_tier") or "fullraw_search"} if str(item.get("canonical_phrase") or "").strip() else None)})
    return out
