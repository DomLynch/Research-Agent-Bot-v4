...
def test_fullraw_fallback_polls_until_complete_receipt(monkeypatch: Any) -> None:
    ...
    assert [call["queue_if_missing"] for call in calls] == [True, True]
...
def test_fullraw_fallback_default_budget_outlives_request_timeout(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={
            "meta": {"shard_receipt": _fullraw_receipt()},
            "results": [{"title": "Queued sweep completed paper"}],
        })

    with httpx.Client(transport=httpx.MockTransport(handler)) as c:
        papers = td._fetch_topic_papers(
            "business_model_performance", client=c, settings=_settings(),
        )

    assert calls == 3
    assert papers[0]["title"] == "Queued sweep completed paper"


def test_fullraw_fallback_budget_accounts_for_request_timeout(
    monkeypatch: Any,
) -> None:
    from agent import topic_discovery as td
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        ...
        calls += 1
        return httpx.Response(200, json={
            "meta": {"async_sweep": {"status": "queued"}, "shard_receipt": {}},
            "results": [{"title": "Not yet trustworthy"}],
        })
...
def test_fullraw_fallback_uses_top_level_full_sweep_receipt(
    ...
    monkeypatch.setenv("TOPIC_DISCOVERY_FULLRAW_POLL_SECONDS", "0")
    monkeypatch.setenv("TOPIC_DISCOVERY_FULLRAW_POLL_ATTEMPTS", "3")
    calls: list[dict[str, Any]] = []
...
def test_discover_topics_reserves_seed_probe_window_with_large_derived_pool(
    monkeypatch: Any, tmp_path: Path,
) -> None:
    from agent import topic_discovery as td
    ...
    monkeypatch.setattr(td, "_SUPPLY_CACHE_PATH", tmp_path / "supply.json")
    monkeypatch.setattr(td, "_title_topic_slugs", lambda *_args, **_kw: [
        "derived_one", "derived_two", "derived_three", "derived_four",
    ])
...
def test_discover_topics_skips_latest_no_signal_title_derived_candidate(
    monkeypatch: Any, tmp_path: Path,
) -> None:
    from agent import topic_discovery as td

    monkeypatch.setattr(td, "_SUPPLY_CACHE_PATH", tmp_path / "supply.json")
    _write_direct_a_core_run(tmp_path, "bad_title_slug", 5)
    (tmp_path / "bad_title_slug-evidence-ts" / "publish_verdict.json").write_text(
        json.dumps({
            "publish_tier": "TIER_3",
    ...
    monkeypatch.setattr(td, "_title_topic_slugs", lambda *_args, **_kw: (
        "bad_title_slug", "good_title_slug",
    ))
    ...
    def fake_counts(topics: list[str], **_: Any) -> dict[str, int]:
        probed.extend(topics)
        return {topic: 6 for topic in topics}
    ...
    assert "bad_title_slug" not in topics
    assert "good_title_slug" in topics
    assert ("good_title_slug",) in fetched
...
def test_discover_topics_counts_slug_prefix_fact_sources(
    ...
    def handler(req: httpx.Request) -> httpx.Response:
        if req.url.path.endswith("/tier2/facts/search"):
            ...
            assert body["domain"] == "longevity"
            query = str(body.get("query") or "")
            if "omega 3" not in query or "longevity" in query:
                return httpx.Response(200, json=[])
            return httpx.Response(200, json=[
                {
                    "id": f"f{i}", "paper_id": f"10.2/{i}",
...
def test_discover_topics_skips_latest_no_signal_fact_child_candidate(
    monkeypatch: Any, tmp_path: Path,
) -> None:
    from agent import topic_discovery as td
    ...
    monkeypatch.setattr(td, "_SUPPLY_CACHE_PATH", tmp_path / "supply.json")
    monkeypatch.setattr(td, "_title_topic_slugs", lambda *_args, **_kw: ())
    _write_direct_a_core_run(tmp_path, "bad_fact_child", 5)
    (tmp_path / "bad_fact_child-evidence-ts" / "publish_verdict.json").write_text(
        json.dumps({
            "publish_tier": "TIER_3",
            "blockers": ["blocked_label:no_signal"],
        }),
    ...
    def fake_fact_counts(
        ...
        kwargs["child_source_counts"]["bad_fact_child"] = 7
        kwargs["child_source_counts"]["good_fact_child"] = 7
        return {"seed_topic": 1}
    ...
    out = td.discover_topics(
        seeds=("seed_topic",), settings=_settings(), client=httpx.Client(),
        current_year=2024, fact_probe_topics=3,
    )

    topics = {candidate.topic for candidate in out}
    assert "bad_fact_child" not in topics
    assert "good_fact_child" in topics
...
def test_cached_source_rich_topics_warms_children_from_latest_run(
    monkeypatch: Any, tmp_path: Path,
) -> None:
    from agent import topic_discovery as td
...
def test_cached_source_rich_topics_does_not_warm_noise_children(
    ...
    facts = [
        {
            "id": f"fact-{i}",
            "paper_id": f"paper-{i}",
            "source_paper": {"doi": f"10.1/noise-{i}", "title": f"Noise trial {i}"},
            "numeric_value": 10,
            "units": "%",
            "population": "adults",
            "intervention": "target intervention",
    ...
    run.joinpath("all_facts.json").write_text(json.dumps(facts), encoding="utf-8")
    run.joinpath("fact_lanes.json").write_text(json.dumps({
        "verdicts": [{"fact_id": f"fact-{i}", "lane": "C_noise"} for i in range(5)],
    }), encoding="utf-8")
