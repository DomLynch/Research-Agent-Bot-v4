...
def test_business_sweep_fullraw_probe_waits_for_admitted_async_completion(
    ...
    sleeps: list[float] = []
    monkeypatch.setenv("V5_MEMO_FULL_RAW_INDEX_TOKEN", "tok")
    monkeypatch.setenv("TOPIC_DISCOVERY_BUSINESS_FULLRAW_QUEUE_RETRY_SECONDS", "2")
    ...
    result = sweep._strict_fullraw_probe("firm_performance", runs_root=tmp_path / "runs")

    assert calls == ["firm performance", "firm performance"]
    assert sleeps == [0.1]
...
def test_business_sweep_fullraw_backoff_shares_business_outcome_family(
    tmp_path: Path, monkeypatch: Any,
) -> None:
    import agent.topic_discovery as topic_discovery_mod
    import scripts.run_topic_discovery as discovery
    ...
    monkeypatch.setenv("TOPIC_DISCOVERY_BUSINESS_FULLRAW_PRIORITY", "1")
    monkeypatch.setenv("TOPIC_DISCOVERY_BUSINESS_FULLRAW_BACKOFF_SECONDS", "3")
    monkeypatch.setenv("TOPIC_DISCOVERY_BUSINESS_FULLRAW_FOREGROUND_SECONDS", "60")
    monkeypatch.setenv("TOPIC_DISCOVERY_BUSINESS_FULLRAW_QUEUE_RETRY_SECONDS", "0")
    monkeypatch.setenv(
        "TOPIC_DISCOVERY_BUSINESS_FULLRAW_LOCK_PATH",
        str(tmp_path / "fullraw.lock"),
    ...
    first = sweep._strict_fullraw_probe(
        "platform_strategy_network_sales", runs_root=tmp_path / "runs",
    )
    sibling = sweep._strict_fullraw_probe(
        "platform_strategy_network_profitability", runs_root=tmp_path / "runs",
    )

    assert first["status"] == "incomplete_receipt"
    assert sibling["status"] == "busy"
    assert sibling["reason"] == "fullraw_backoff"
    assert sibling["backoff_key"] == "platform strategy network"
    assert sibling["backoff_seconds"] == 60
    assert calls == ["platform_strategy_network_sales".replace("_", " ")]
...
def test_business_sweep_fullraw_probe_sheds_unadmitted_full_queue_without_sleep(
    tmp_path: Path, monkeypatch: Any,
) -> None:
    import agent.topic_discovery as topic_discovery_mod
    import scripts.run_topic_discovery as discovery
...
def test_business_sweep_fullraw_probe_sheds_in_progress_cache_hit_without_sleep(
    ...
    monkeypatch.setattr(
        sweep,
        "_business_fullraw_queries",
        lambda _topic: ("employee engagement turnover sales",),
    )
    monkeypatch.setattr(
        "scripts.run_business_alpha_sweep.time.sleep",
        lambda seconds: sleeps.append(seconds),
    ...
    result = sweep._strict_fullraw_probe(
        "employee_engagement_turnover_sales",
        include_papers=True,
    )
...
def test_business_sweep_retries_soft_source_floor_family(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    soft_topic = "pricing_strategy_margin"
    fetched_topics: list[str] = []
    ...
    monkeypatch.setattr(sweep, "_DOMAINS", ("business_research",))
    monkeypatch.setattr(sweep, "load_domain_profile", lambda _domain: profile)
    monkeypatch.setattr(sweep, "_seed_topics", lambda _path, *, limit: [soft_topic])
    monkeypatch.setattr(sweep, "_prioritized_seed_topics", lambda _root, _domain, topics: topics)
    monkeypatch.setattr(sweep, "_recent_source_literature_blocked_topics", lambda *_args: {
        soft_topic,
    })
    monkeypatch.setattr(cycle, "_recent_source_floor_topics", lambda *_args, **_kwargs: {
        soft_topic,
    })
    monkeypatch.setattr(cycle, "_recent_source_literature_structural_blocked_topics", lambda *_args, **_kwargs: set())
    monkeypatch.setattr(cycle, "_priority_source_literature_repair_decisions", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(cycle, "_repairable_source_literature_topics", lambda *_args, **_kwargs: [])
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-05T17-45-00Z",
    ])

    assert sweep.main() == 2
    assert fetched_topics == [soft_topic]
...
def test_business_sweep_promotes_cached_ready_beyond_rank_window_without_fullraw_queue(
    ...
    def papers_for(topic: str) -> list[dict[str, Any]]:
        outlets = (
            ...
            "Gamma Network Letters",
            "Delta Business Quarterly",
            "Epsilon Performance Studies",
        )
        return [
            {
                "title": f"{topic.replace('_', ' ')} source {idx}",
                "doi": f"10.9393/{topic}-{idx}",
                "journal_name": outlets[idx],
                "abstract": (
                    f"{topic.replace('_', ' ')} significantly improves bounded "
                    f"firm performance outcome {idx}."
                ),
                "source_fact": {
                    "canonical_phrase": (
                        f"{topic.replace('_', ' ')} significantly improves "
                        f"bounded firm performance outcome {idx}"
                    ),
                    "population": "firms",
                    "intervention": topic.replace("_", " "),
                    "endpoint": f"firm performance outcome {idx}",
    ...
    def cached_ready(
        _runs_root: Path, _domain: str, topic: str, _settings: Any,
    ) -> tuple[list[dict[str, Any]], str]:
        cache_ready_calls.append(topic)
        if topic == ready_topic:
            return papers_for(topic), topic
        return [], ""
    ...
    def fake_run_cycle(**kwargs: Any) -> dict[str, Any]:
        submissions.append(kwargs)
        forced = kwargs["source_literature_forced_papers"][ready_topic]
        assert len(forced) == 5
        assert publish_literature.substantive_fact_count(forced) == 5
        assert publish_literature.source_identity_count(
            forced, require_substantive=True,
    ...
    monkeypatch.setattr(
        sweep,
        "_prioritized_seed_topics",
        lambda _root, _domain, seed_topics: seed_topics,
    )
    monkeypatch.setattr(cycle, "_priority_source_literature_repair_decisions", lambda *_args, **_kwargs: {})
    monkeypatch.setattr(cycle, "_repairable_source_literature_topics", lambda *_args, **_kwargs: [])
    monkeypatch.setattr(sweep, "_cached_fullraw_complete_hit_count", lambda _topic: 0)
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-00-00Z",
    ])

    assert sweep.main() == 0
    assert ready_topic in cache_ready_calls
    assert fullraw_calls == []
    assert submissions[0]["source_literature_forced_papers"] == {
        ready_topic: papers_for(ready_topic),
    }


def test_business_sweep_promotes_ready_discovery_cache_outside_seed_pool(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    seed_topic = "duplicate_strategy_performance"
    ready_topic = "new_market_entry_productivity"
    cache_ready_calls: list[str] = []
    submissions: list[dict[str, Any]] = []
    discovery_dir = runs_root / "_topics_discovery"
    discovery_dir.mkdir(parents=True)
    (
        discovery_dir / f"business_sweep_fullraw.business_research.{ready_topic}.json"
    ).write_text(
        json.dumps({"all": [{
            "topic": ready_topic,
            "paper_count": 5,
    ...
    def papers_for(topic: str) -> list[dict[str, Any]]:
        outlets = (
            ...
            "Gamma Network Letters",
            "Delta Business Quarterly",
            "Epsilon Productivity Studies",
        )
        return [
            {
                "title": f"{topic.replace('_', ' ')} source {idx}",
                "doi": f"10.9696/{topic}-{idx}",
                "journal_name": outlets[idx],
                "abstract": (
                    f"{topic.replace('_', ' ')} significantly improves bounded "
                    f"firm productivity outcome {idx}."
                ...
                "source_fact": {
                    ...
                    "population": "firms",
                    "intervention": topic.replace("_", " "),
                    "endpoint": f"productivity outcome {idx}",
                    "source_tier": "fullraw_search",
                },
            }
            for idx in range(5)
    ...
    monkeypatch.setattr(
        sweep,
        "_strict_fullraw_probe",
        lambda topic, *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError(f"live fullraw should not run before cache supply: {topic}"),
        ),
    )
    monkeypatch.setattr(sweep, "run_cycle", fake_run_cycle)
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-30-00Z",
    ])
...
def test_business_sweep_skips_submitted_discovery_cache_source_set(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    seed_topic = "duplicate_strategy_performance"
    duplicate_topic = "supply_chain_resilience_productivity"
    fresh_topic = "platform_strategy_network_productivity"
    cache_ready_calls: list[str] = []
    submissions: list[str] = []
    discovery_dir = runs_root / "_topics_discovery"
    ...
    def write_discovery(topic: str) -> None:
        (discovery_dir / f"business_sweep_fullraw.business_research.{topic}.json").write_text(
            json.dumps({"all": [{
                "topic": topic,
                "paper_count": 5,
    ...
    write_discovery(fresh_topic)
    write_discovery(duplicate_topic)
    submitted_dir = runs_root / "_daily_ledger"
    submitted_dir.mkdir(parents=True)
    (submitted_dir / "_submitted_fingerprints.json").write_text(
        json.dumps([{
            "domain_slug": "business_research",
            "topic": duplicate_topic,
            "bundle_signature": sweep._source_literature_bundle_signature(
                papers_for(duplicate_topic),
            ),
        }]),
        encoding="utf-8",
    )
    ...
    def cached_ready(
        ...
        cache_ready_calls.append(topic)
        if topic in {duplicate_topic, fresh_topic}:
            return papers_for(topic), topic
        return [], ""

    def fake_run_cycle(**kwargs: Any) -> dict[str, Any]:
        forced = kwargs.get("source_literature_forced_papers") or {}
        topic = next(iter(forced), "")
        submissions.append(str(topic))
        assert topic == fresh_topic
        return {"status": "submitted_to_researka", "submitted": 1, "published": 0}
    ...
    monkeypatch.setattr(
        ...
        "_strict_fullraw_probe",
        lambda topic, *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError(f"duplicate cache should not force live fullraw: {topic}"),
        ),
    )
    monkeypatch.setattr(sweep, "run_cycle", fake_run_cycle)
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-35-00Z",
    ])

    assert sweep.main() == 0
    assert duplicate_topic in cache_ready_calls
    assert fresh_topic in cache_ready_calls
    assert submissions == [fresh_topic]


def test_business_sweep_widens_ranked_topics_when_cache_supply_is_exhausted(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    stale_topic = "stale_strategy_performance"
    weak_topic = "weak_market_productivity"
    live_topic = "fresh_channel_productivity"
    ...
    def papers_for(topic: str) -> list[dict[str, Any]]:
        outlets = (
            "Alpha Strategy Review",
            "Beta Channel Journal",
            "Gamma Market Letters",
            "Delta Business Quarterly",
            "Epsilon Productivity Studies",
        )
        return [
    ...
    def fake_fullraw_probe(
        topic: str, *_args: Any, **_kwargs: Any,
    ) -> dict[str, Any]:
        probe_calls.append(topic)
        if topic == live_topic:
            return {
                "status": "complete",
        ...
        return {"status": "queue_saturated", "paper_count": 0, "_papers": []}

    def fake_run_cycle(**kwargs: Any) -> dict[str, Any]:
        forced = kwargs.get("source_literature_forced_papers") or {}
        topic = next(iter(forced), "")
        submissions.append(str(topic))
        assert topic == live_topic
        forced_papers = forced[live_topic]
        assert len(forced_papers) == 5
        assert publish_literature.substantive_fact_count(forced_papers) == 5
        assert publish_literature.source_identity_count(
            forced_papers, require_substantive=True,
        ) == 5
        assert sweep._source_outlet_count(forced_papers) == 5
        return {"status": "submitted_to_researka", "submitted": 1, "published": 0}
    ...
    monkeypatch.setattr(
        sweep,
        "_seed_topics",
        lambda _path, *, limit: [stale_topic, weak_topic, live_topic],
    )
    monkeypatch.setattr(
        sweep,
        "_prioritized_seed_topics",
    ...
    monkeypatch.setattr(
        sweep,
        "_cached_source_literature_discovery_topics",
        lambda *_args, **_kwargs: [],
    )
    monkeypatch.setattr(
        sweep,
    ...
    monkeypatch.setattr(sweep, "_fullraw_search_hits", lambda _query: [])
    monkeypatch.setattr(sweep, "_strict_fullraw_probe", fake_fullraw_probe)
    monkeypatch.setattr(sweep, "run_cycle", fake_run_cycle)
    monkeypatch.setattr(sys, "argv", [
        "run_business_alpha_sweep.py",
        "--cycles", "1",
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-40-00Z",
    ])

    assert sweep.main() == 0
    assert probe_calls == [stale_topic, weak_topic, live_topic]
    assert submissions == [live_topic]


def test_business_sweep_uses_cached_ready_backup_after_duplicate_repair(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    repair_topic = "digital_transformation_firm"
    topics = [repair_topic, *(f"candidate_{idx}_performance" for idx in range(6))]
    ready_topic = topics[-1]
    ...
    def cached_ready(
        ...
        nonlocal repair_finished
        if topic == ready_topic and not repair_finished:
            raise AssertionError("fresh cache backup should not outrank repair")
        if topic in {repair_topic, ready_topic}:
            return papers_for(topic), topic
        return [], ""
    ...
    def fake_run_cycle(**kwargs: Any) -> dict[str, Any]:
        ...
        assert topic == ready_topic
        forced_papers = forced[ready_topic]
        assert len(forced_papers) == 5
        assert publish_literature.substantive_fact_count(forced_papers) == 5
        assert sweep._source_outlet_count(forced_papers) == 5
        return {"status": "submitted_to_researka", "submitted": 1, "published": 0}
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-10-00Z",
    ])

    assert sweep.main() == 0
    assert submissions == [repair_topic, ready_topic]
    assert fullraw_calls == []


def test_business_sweep_skips_duplicate_cached_repair_before_submit(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    repair_topic = "supply_chain_resilience_margin"
    ready_topic = "platform_strategy_network_performance"
    topics = [repair_topic, ready_topic]
    submissions: list[str] = []
    ...
    monkeypatch.setattr(cycle, "_pending_source_literature_topics", lambda *_a, **_k: set())
    monkeypatch.setattr(sweep, "_cached_fullraw_complete_hit_count", lambda _topic: 0)
    monkeypatch.setattr(
        sweep,
        "_cached_ready_source_literature_papers_with_topic",
    ...
    monkeypatch.setattr(
        sweep,
        "_strict_fullraw_probe",
        lambda topic, **_kwargs: (_ for _ in ()).throw(
            AssertionError(f"duplicate repair should not force live fullraw: {topic}"),
        ),
    )
    monkeypatch.setattr(sweep, "run_cycle", fake_run_cycle)
    monkeypatch.setattr(sys, "argv", [
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "1",
        "--submit-date", "2026-07-01T10-15-00Z",
    ])

    assert sweep.main() == 0
    assert submissions == [ready_topic]


def test_business_sweep_waits_on_cached_backup_without_returning_to_fullraw(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    profile = load_domain_profile("business_research")
    runs_root = tmp_path / "runs"
    repair_topic = "digital_transformation_firm"
    live_fullraw_topic = "platform_strategy_network_profitability"
    ready_topic = "supply_chain_resilience_productivity"
    topics = [repair_topic, live_fullraw_topic, ready_topic]
    repair_finished = False
    submissions: list[str] = []
    ...
    def fail_fullraw(topic: str, **_kwargs: Any) -> dict[str, Any]:
        raise AssertionError(f"live fullraw should not run after cache backup: {topic}")
    ...
    monkeypatch.setattr(sweep, "_strict_fullraw_probe", fail_fullraw)
    monkeypatch.setattr(sweep, "run_cycle", fake_run_cycle)
    monkeypatch.setattr(sys, "argv", [
        "run_business_alpha_sweep.py",
        "--cycles", "1",
        ...
        "--runs-root", str(runs_root),
        "--submit-after-consistent-passes", "2",
        "--submit-date", "2026-07-01T10-20-00Z",
    ])

    assert sweep.main() == 2
    assert submissions == [repair_topic]
    consistency = json.loads(
        (
            runs_root / "_business_diagnostics"
            / "ready_consistency.business_research.json"
        ).read_text(encoding="utf-8"),
    )
    assert consistency[-1]["topic"] == ready_topic
    assert consistency[-1]["passes"] == 1
