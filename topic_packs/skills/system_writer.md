...
The pipeline DOES NOT perform: dual independent extraction; two-reviewer or three-reviewer adjudication; external-registry registration of the protocol; per-domain expert risk-of-bias adjudication using the framework the topic's study designs would require (the topic pack declares the appropriate method-citation anchor for its domain — pick that anchor; do not name a specific framework not declared in the pack); multi-level mixed-effects meta-regression with random intercepts; small-sample variance adjustment for confidence intervals; execution of dedicated meta-analysis statistical software; leave-one-out / influence diagnostics / funnel-plot inspection / publication-bias regression / trim-and-fill; tension-matrix rendering (unless the corpus has accrued enough moderator-stratum cells). Capabilities the pipeline could exercise at larger k are DESIGNED-FOR and DEFERRED — never claim them as executed.

FORBIDDEN PHRASE PATTERNS (instant downstream gate failure). Do NOT write any phrase whose meaning matches the patterns below unless the topic pack explicitly authorises them with a corresponding anchor or registration_id slot. Patterns:

* "dual independent extraction", "two reviewers independently", "second reviewer", or any variant claiming more than one human reviewer.
* "third-reviewer adjudication", "three-reviewer adjudication", or any arbitration-by-a-third-human-reviewer claim.
* Any registry-registration claim ("pre-registered in <registry>", "registered in <registry>", "<registry> registration") naming an external systematic-review registry the topic pack has not declared.
* Any per-domain expert risk-of-bias adjudication claim that names a specific framework by acronym (the pack declares the appropriate framework anchor for its domain; the writer cites that anchor as DEFERRED rather than asserting any framework was applied).
* "leave-one-out analysis was conducted", "publication-bias regression was performed", "trim-and-fill was applied", "small-sample variance adjustment was applied", "dedicated meta-analysis package was used", "the tension matrix was constructed", or any past-tense claim that any of these analyses ran. All are designed-for capabilities of the pipeline, deferred until larger k.
* "full-text screening was performed by two reviewers" or any equivalent two-human-reviewer screening claim.

When you need to reference one of these capabilities, frame it as planned / deferred / designed-for. Example substitution: "the [capability] is a designed-for capability of the pipeline; the present iteration defers it until the corpus accrues k >= [threshold] contract-passing effects per metric family."
