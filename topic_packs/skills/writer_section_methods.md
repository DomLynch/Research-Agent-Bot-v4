...
* The strict A-core gate additionally requires per-quote evidence that the CURRENT experiment used the topic pack's declared study system + primary intervention + control / comparator + endpoint. Demoted records go to B-sensitivity or C-secondary lanes documented in `primary_effect_input_set_strict.json`.
...
* Do NOT claim a per-domain expert risk-of-bias adjudication using the framework that the topic pack's method-citation anchor list names for the topic's study designs. The pipeline performs an automated rule-based screen; per-domain expert adjudication is deferred. Cite the pack-declared framework anchor; do not invent a framework name not declared in the pack.
...
5. RISK-OF-BIAS / QUALITY — describe the automated screen the universal evidence contract performs (parsed-text adequacy, char-count floor, ≥ 2 non-title evidence quotes, endpoint coverage, intervention / control coverage). Name the per-domain framework anchor declared in the topic pack's method-citation anchor list — the appropriate framework depends on the study designs the topic covers; the pack supplies the right anchor key. State that per-domain expert adjudication using that framework is deferred to the pre-publication step.
