...
* Do NOT claim a per-domain expert risk-of-bias adjudication using the topic's relevant framework (e.g., SYRCLE for animal-intervention studies, Cochrane RoB 2 for randomised human trials, ROBINS-I for non-randomised human studies, or the equivalent for non-biomedical domains). The pipeline performs an automated rule-based screen; per-domain expert adjudication is deferred.
