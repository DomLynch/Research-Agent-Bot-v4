#!/usr/bin/env bash
# LOC gate — fail if agent/ exceeds the 7,500 LOC ceiling.
...
# + supplement generator). Every new module under the higher cap must delete
# or prevent a fake-evidence failure mode (receipts, validators, provenance,
# typed contracts), not buy prose polish or speculative abstraction.
set -euo pipefail

CEILING="${LOC_CEILING:-7500}"
