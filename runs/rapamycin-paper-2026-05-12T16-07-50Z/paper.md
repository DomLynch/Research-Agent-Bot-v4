<!-- AUTO-STITCHED Sprint 11.2 — placeholder + MODERATOR_P + honesty rewrites + citation resolution + honest back-matter.
  placeholders resolved: 12 (unresolved: [])
  honesty rewrites applied: 9
  citations resolved: 19 (unresolved: [])
  stamped: 2026-05-12T16:07:50+00:00
-->
...
This synthesis will be implemented using a reproducible evidence-contract pipeline and is reported against PRISMA 2020 where applicable [15]. The search strategy will query PubMed, OpenAlex, Europe PMC, Crossref, Unpaywall, Semantic Scholar, CORE, OSF Preprints, bioRxiv/medRxiv, ClinicalTrials.gov, and the Researka internal canonical index (16 sources aggregated per the universal retrieval contract) from January 2001 (post-Easter-Island rapamycin era) through April 2026 (final retrieval cycle). The query-term structure will combine intervention terms (e.g., rapamycin, sirolimus, mTOR inhibitor) with lifespan-related outcomes and model organisms (e.g., survival, longevity, mouse, mice). The full query syntax is available as a Boolean composition (rapamycin OR sirolimus) AND (lifespan OR survival OR longevity OR mortality) AND (mouse OR mice OR "Mus musculus"); full Boolean syntax filed in supplement S1.
